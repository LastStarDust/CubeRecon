...
class Cube::TEventChangeManager: public TObject {
public:
    TEventChangeManager();
    ...
    void AddNewEventHandler(Cube::TVEventChangeHandler* handler);

    /// Add a handler (taking ownership of the handler) for when the event
    /// needs to be redrawn.  These handlers are executed by the UpdateEvent()
    /// method.
    void AddUpdateHandler(Cube::TVEventChangeHandler* handler);
...
private:
    ...
    typedef std::vector<Cube::TVEventChangeHandler*> Handlers;
